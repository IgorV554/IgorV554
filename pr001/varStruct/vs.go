...
// --------------------------------------------------------------
// Передаем структуру в функцию по указателю
// Область видимости: поля тоже должны нач на ЗАГЛАВНУЮ!
type Ms struct {
	Am, Bm, Cm int32
}

func FFvs6(ms *Ms) { // указатель на структуру - меняем структуру а не ее экземпляр
	fmt.Println("---------Интерфейсы---------")
	mul := func() int32 { ms.Cm = ms.Am * ms.Bm; return ms.Cm } // аноним ф-я
	fmt.Println(mul())                                          // 10000                                        // 450
}

/* применение в мэйн
var u vs.Ms = vs.Ms{Am: 100, Bm: 100, Cm: 0}
inf.Finf01(&u) // передача указателя ее в Ф
fmt.Println(u) // {100 100 10000} проверка что члены структуры изменены - ДА!
*/
